...
if($do == 'default') {
	...
	$now=time();
	$vipnum = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('ewei_shop_member') . ' as m,'. tablename('mc_mapping_fans')." as  f  WHERE m.uniacid = :uniacid and m.viptime>{$now} and m.uniacid=f.uniacid and m.uid=f.uid AND f.acid = :acid", array(':uniacid' => $_W['uniacid'], ':acid' => $_W['acid']));	
	template('mc/mass');
}
...
if($do == 'post') {
	...
	if($post['msg_type'] == 'video') {
		...
		if($post['send_type'] == '3' || $post['send_type'] == '4') {
			$data['video'] = array(
				'media_id' => $status['media_id'],
			);
			$data['msgtype'] = 'video';
	...
	}elseif($post['send_type'] == 4) {
		//VIP群发
		$now=time();
		$openidlist = pdo_fetchall('SELECT f.openid FROM ' . tablename('ewei_shop_member') . ' as m,'. tablename('mc_mapping_fans')." as  f  WHERE m.uniacid = :uniacid and m.viptime>{$now} and m.uniacid=f.uniacid and m.uid=f.uid AND f.acid = :acid", array(':uniacid' => $_W['uniacid'], ':acid' => $_W['acid']));	
		$post['openids']=array();
		foreach($openidlist as $op){			
			$post['openids'][]=$op['openid'];	
		}		
		$data['touser'] = $post['openids'];
	}
	$record = 0;
